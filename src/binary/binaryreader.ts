...
export class BinaryReader {
  ...
  private _ReadArray(bytes: number): Uint8Array {
    const arr = this.buffer.slice(this.offset, this.offset + bytes);
    this.offset += bytes;
    return new Uint8Array(arr);
  }

  private _ReadString(bytes: number): string {
    const str = this.buffer.toString("utf8", this.offset, this.offset + bytes);
    this.offset += bytes;
    return str;
  }
  ...
  ReadUInt8(): number {
    const number = this.buffer.readUInt8(this.offset, this.noAssert);
    this.offset += 1;
    return number;
  }

  ReadInt8(): number {
    const number = this.buffer.readInt8(this.offset, this.noAssert);
    this.offset += 1;
    return number;
  }

  ReadUInt16(): number {
    const number = this.littleEndian
