...
export async function readItem(reader: BinaryReader, version: number, constants: types.IConstantData, config: types.IConfig, parent?: types.IItem): Promise<types.IItem> {
  ...
  if (!item.simple_item) {
      item.id = _readBits(reader, start, offset, 32); offset+=32;
      item.level = _readBits(reader, start, offset, 7); offset+=7;
      item.quality = _readBits(reader, start, offset, 4); offset+=4;
      item.multiple_pictures = _readBits(reader, start, offset, 1); offset+=1;
...
function _readSimpleBits(item: types.IItem, reader: BinaryReader, version: number, constants: types.IConstantData, config: types.IConfig) : number {
  let start = reader.Position();
  //1.10-1.14d 
  ...
  if(item.is_ear) {
    ...
    if(version <= 0x60) {
      ...
      for (let i = 0; i < arr.length; i++) {
          arr[i] = _readBits(reader, start, offset, 8); offset += 8;
      }
      ...
      for (let i = 0; i < 4; i++) {
        ...
        do {
          node = node[_readBits(reader, start, offset, 1)]; offset+=1;
    ...
    let l = item.simple_item ? 1 : 3;
    item.nr_of_items_in_sockets = _readBits(reader, start, offset, l);offset+=l;
  }
  return offset;
}
...
function _writeSimpleBits(writer: BinaryWriter, version: number, item: types.IItem, constants: types.IConstantData, config: types.IConfig) : number {
  var start = writer.Position();
  ...
  _writeBits(writer, item.position_x, start, offset, 4); offset+=4;
  _writeBits(writer, item.position_y, start, offset, 4); offset+=4;
  _writeBits(writer, item.alt_position_id, start, offset, 3); offset+=3;
  if (item.is_ear) {
    _writeBits(writer, item.ear_attributes.class, start, offset, 3); offset+=3;
    _writeBits(writer, item.ear_attributes.level, start, offset, 7); offset+=7;
    let name = item.ear_attributes.name.substring(0, 15);
    for (let i = 0; i < name.length; i++) {
        _writeBits(writer, name.charCodeAt(i) & 0x7f, start, offset, 7); offset+=7;
    }
    _writeBits(writer, 0x00, start, offset, 7); offset+=7;
  } else {
    let t = item.type.padEnd(4, " ");
    if(version <= 0x60) {
      let type = new BinaryWriter(4).SetLittleEndian()
          .WriteString(t)
          .toArray();
      _writeBits(writer, type, start, offset, 32); offset+=32;
    } else {
      for(let c of t) {
        let n = HUFFMAN_LOOKUP[c];
        _writeBits(writer, n.v, start, offset, n.l); offset+=n.l;
      }
    }
    let l = item.simple_item ? 1 : 3;
    _writeBits(writer, item.nr_of_items_in_sockets, start, offset, l); offset+=l;
  }
  return offset;
}
